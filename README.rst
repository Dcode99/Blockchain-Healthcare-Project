...
- When adding a new peer, generate new public and private keys using the keypair.py program. Rename these files and replace the existing key files in their respective folders, then replcae the public key shown in the genesis blocks with the new keys.
